...
$lines[] = $_POST['fname']."<>".
		   $_POST['savefile']."<>".
		   $_POST['datedata'][0]."/".$_POST['datedata'][1]."/".$_POST['datedata'][2]." ".str_pad($_POST['datedata'][3], 2, '0', STR_PAD_LEFT).":".str_pad($_POST['datedata'][4], 2, '0', STR_PAD_LEFT)."<>".
		   $_POST['dtype']."<>".
		   $_POST['price']."<>".
		   $_POST['birthday']."<>".
		   $_POST['sex']."<>".
...
$html['topic'] = "<form name=\"infoform\" action=\"admin.php\" method=\"post\">
	<select name=\"dtype\" onchange=\"document.infoform.submit()\">
		<option value=\"\" ${selected0}>全て</option>
		<option value=\"ワイマラナー\" ${selected2}>ワイマラナー</option>
		<option value=\"柴犬\" ${selected3}>柴犬</option>
		</select><br></form>";
...
$upimage$upimage2$upimage3$upimage4
$data[12]
</td>\n
	...
	for($i=0;$i<$totallines;$i++){
		...
		if($data[0] == $mark){
			//if($_POST['mode'] == "renewend"){
				...
				$data[6] = $_POST['sex'];
				$data[7] = $_POST['color'];
				$data[8] = $_POST['pedigree'];
				$data[9] = $_POST['birthplace'];
				$data[10] = $_POST['breeder'];
				$data[11] = $_POST['stocking'];
				$data[12] = $_POST['movie'];
		...
		if($data[0]){
			...
			$html = str_replace("\$price", $data[4], $html);
			$html = str_replace("\$birthday", $data[5], $html);
			$html = str_replace("\$sex", $data[6], $html);
			$html = str_replace("\$color", $data[7], $html);
			$html = str_replace("\$pedigree", $data[8], $html);
			$html = str_replace("\$birthplace", $data[9], $html);
			$html = str_replace("\$breeder", $data[10], $html);
			$html = str_replace("\$stocking", $data[11], $html);
			$html = str_replace("\$movie", $data[12], $html);
...
if(!$_POST['price']){
$errmsg .= "価格を入力いただいたかご確認ください<br>";
}

if(!$_POST['birthday']){
$errmsg .= "生年月日を入力いただいたかご確認ください<br>";
}

if(!$_POST['sex']){
$errmsg .= "性別を入力いただいたかご確認ください<br>";
}

if(!$_POST['color']){
$errmsg .= "カラーを入力いただいたかご確認ください<br>";
}

if(!$_POST['pedigree']){
$errmsg .= "血統書を入力いただいたかご確認ください<br>";
}

if(!$_POST['birthplace']){
$errmsg .= "出生地を入力いただいたかご確認ください<br>";
}

if(!$_POST['breeder']){
$errmsg .= "繁殖者を入力いただいたかご確認ください<br>";
}

if(!$_POST['stocking']){
$errmsg .= "仕入者を入力いただいたかご確認ください<br>";
}


if(count(preg_grep("/^[0-9]+$/", $_POST['datedata'])) != 5){
$errmsg .= "作成日時の入力をご確認ください";
}
