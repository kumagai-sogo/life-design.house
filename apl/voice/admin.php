...
for($i=1;$i<=$upimage_setcnt;$i++) {
	if($_POST['savefile'.$i]){
		...
		if($imgsize[2] == 1){
			$originalimage = imagecreatefromgif($images_dir.$_POST['savefile'.$i]);
		}elseif($imgsize[2] == 2){
			$originalimage = imagecreatefromjpeg($images_dir.$_POST['savefile'.$i]);
		}elseif($imgsize[2] == 3){
			$originalimage = imagecreatefrompng($images_dir.$_POST['savefile'.$i]);
		}

		list($w, $h) = getimagesize($images_dir.$_POST['savefile'.$i]);
...
$thumbH = 400;
		...
		imagecopyresampled($new_image, $originalimage, 0, 0, $diffX, $diffY, $thumbW, $thumbH, $diffW, $diffH);

		if($imgsize[2] == 1){
			imagegif($new_image, $images_dir.$_POST['savefile'.$i]);
		}elseif($imgsize[2] == 2){
			imagejpeg($new_image, $images_dir.$_POST['savefile'.$i], 100);
		}elseif($imgsize[2] == 3){
			imagepng($new_image, $images_dir.$_POST['savefile'.$i], 6);
		}
			...
			//   $_POST['image'.$i] = imageconvolution($image, $matrix, $divisor, 0);
			// }
...
switch($data[3]) {
	...
	case "2":
		...
		break;
	case "3":
		$dtypechkeck4 = "checked";
		break;
	case "4":
		$dtypechkeck4 = "checked";
		break;
	case "共通":
		$dtypechkeck6 = "checked";
		break;
	default:
		break;
}
...
for($i=0,$j=0;$i<count($linesall);$i++){
	...
	switch($_POST['dtype']){
		...
		case "2":
			...
			break;
		case "3":
			if($data[3] == "3" || $data[3] == "共通") {
				$lines[$j] = $linesall[$i];
				$j++;
			}
			break;
		case "4":
			if($data[3] == "4" || $data[3] == "共通") {
				$lines[$j] = $linesall[$i];
				$j++;
			}
			break;
